...
import { Log, ShowStatusBar, SetStatusBarText, HideStatusBar } from './utils/log.mjs';
import { CheckForSudoPriviledge } from './utils/cmd.mjs';

async function Main() {
	// Get sudo priviledge
	if(!await CheckForSudoPriviledge()) {
		Log('Not running with admin priviledge, quitting.', 'warn');
		return;
	}
